...
Persona = namedtuple('Persona', [
                     'nombre', 'edad', 'tiene_licencia', 'hobbies', 'paises_visitados', 'libros_leidos'])

juan = Persona('Juan', 20, True, {'fútbol', 'cine'}, {
               'España', 'Chile'}, ['Don Quijote', 'El Principito'])
maria = Persona('Maria', 15, False, {'lectura', 'música', 'cine'}, {
                'Francia', 'Chile'}, ['El Principito', '1984'])
pedro = Persona('Pedro', 17, True, {'fútbol', 'natación'}, {'México'}, [
                'Cien Años de Soledad', 'Moby Dick', '1984'])
...
print("'El Principito' es el último libro que ha leído Maria:",
      len(
          maria.libros_leidos) > 0 and maria.libros_leidos[-1] == "El Principito"
      )
...
print("Juan ha leído más libros que María pero menos que Pedro:",
      ...
      len(pedro.libros_leidos) > len(
          juan.libros_leidos) > len(maria.libros_leidos)
...
print("El último libro que María ha leído es '1984' y Juan no lo ha leído aún:",
      len(maria.libros_leidos) > 0 and maria.libros_leidos[-1] == "1984" and
      "1984" not in juan.libros_leidos
      )

print("Pedro ha visitado más países que Juan o ha leído más libros que María:",
      len(pedro.paises_visitados) > len(juan.paises_visitados) or len(
          pedro.libros_leidos) > len(maria.libros_leidos)
      )

print("María tiene licencia de conducir o ha leído 'Moby Dick', pero no ambas cosas",
      (maria.tiene_licencia or "Moby Dick" in maria.libros_leidos) and not (
...
# TODO: Faltan los 3 últimos, ver enunciado en el README.md
